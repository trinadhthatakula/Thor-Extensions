...
/**
 * Stormbringer's configuration UI, presented as a translucent bottom sheet (the manifest theme is
 ...
 * full Compose/Asgard, so nothing crosses the boundary. All state travels over Stormbringer's own
 * config provider IPC.
 */
...
class ConfigActivity : ComponentActivity() {
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        // launched directly via adb) -> null -> StormbringerTheme falls back to Thor's defaults.
        ...
        setContent {
            StormbringerTheme(themeArgs) {
                StormbringerConfigSheet(onDismiss = { finish() })
            }
        }
    }
}
...
@Composable
private fun StormbringerConfigSheet(onDismiss: () -> Unit) {
        ...
        Column(
            ...
            AsgardSettingToggleRow(
                ...
                subtitle = "Disables Android's signature/tamper protection for installs Thor performs. " +
                        "Requires Root + the Stormbringer LSPosed module active.",
                checked = corePatch,
                onCheckedChange = { v ->
                    if (v) {
            ...
            Column {
                Text(
                    "This disables Android's signature, tamper, and impersonation protection for " +
                            ...
                            "Requires Root and the Stormbringer LSPosed module active.\n\n" +
