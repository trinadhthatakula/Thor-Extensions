...
/**
 * A faithful copy of Thor's "Asgardian" theme so Stormbringer's config UI (which runs in this
 * extension's OWN process and can't read Thor's Compose theme) matches Thor exactly. Thor passes its
 * resolved settings — theme mode, dynamic color, AMOLED — as CONFIGURE-intent extras; [ThemeArgs]
 * carries them and [StormbringerTheme] applies the same logic as Thor's ThorTheme(). Keep the color
 ...
 */
@Composable
fun StormbringerTheme(args: ThemeArgs?, content: @Composable () -> Unit) {
