...
**Status:** COMPLETE
**Commit:** f3cb15a — `feat(stormbringer): config stores installer-spoof package set`
**Build:** `./gradlew :app:assembleDebug` → BUILD SUCCESSFUL (compileDebugKotlin ran clean, no warnings/errors).

## Exact edits to `app/src/main/java/com/valhalla/thor/ext/stormbringer/Config.kt`
...
### 2. Added two branches to the existing `when (method)` in `StormbringerConfigProvider.call` (existing `"get"`/`"set"` boolean branches left untouched):
```kotlin
