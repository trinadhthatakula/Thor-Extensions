...
Strombringer is licensed under the **GNU General Public License, version 2
(GPLv2)**. See the [`LICENSE`](LICENSE) file for the full text.

This module includes and/or derives code from
[CorePatch](https://github.com/LSPosed/CorePatch) (tag `4.9`), which is
licensed under GPLv2 (credit: weishu, LSPosed, yujincheng08). CorePatch v4.9 is
GPLv2-**only** (no "or later" election), so the combined work is distributed
under GPLv2. See [`NOTICE`](NOTICE) for full attribution and the license
provenance finding.
